#!/usr/bin/env python3
"""
Evolution Gaming Roulette Results Collector (Stealth Mode)
Main application file - bypasses Cloudflare and anti-bot protections
"""
...
from roulette_detector_stealth import RouletteDetectorStealth
from discord_notifier import DiscordNotifier
from local_html_client import LocalHTMLClient
from roulette_result import RouletteResult

class RouletteCollectorStealth:
    """Stealth roulette results collector application"""
    
    def __init__(self):
        self.detector = RouletteDetectorStealth()
    ...
    def start(self) -> bool:
        ...
        try:
            self.logger.info("Starting Roulette Results Collector (Stealth Mode)...")
            ...
            if not self.detector.initialize_browser():
                self.logger.error("Failed to initialize browser with stealth mode")
                return False
            ...
            self.logger.info("Roulette Collector started successfully with stealth mode!")
...
def main():
    """Main entry point"""
    print("🎰 Evolution Gaming Roulette Results Collector (Stealth Mode)")
    print("=" * 65)
    print("🛡️  Bypassing Cloudflare and anti-bot protections...")
    print("=" * 65)
    
    collector = RouletteCollectorStealth()
